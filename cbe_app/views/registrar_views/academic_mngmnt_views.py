...
def clone_curriculum(request):
    ...
    try:
        ...
        source_version = (
            CurriculumVersion.objects.filter(academic_year=source_year).first()
            if source_year
            else CurriculumVersion.objects.filter(is_active=True).first()
        )
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_current_academic_year_and_term_view(request):
...
# FIX: competency_detail now handles both PUT and DELETE (was only DELETE before)
@api_view(['PUT', 'DELETE'])
@permission_classes([IsAuthenticated])
def competency_detail(request, competency_id):
    """Update or delete a specific competency"""
    try:
        try:
        ...
        if request.method == 'PUT':
            if request.user.role not in ['registrar', 'system_admin']:
                return Response({
                    'success': False,
                    'error': 'You do not have permission to update competencies'
                }, status=status.HTTP_403_FORBIDDEN)

            serializer = CompetencySerializer(competency, data=request.data, partial=True)

            if serializer.is_valid():
                serializer.save()
                logger.info(f"Competency updated: {competency.competency_code} by {request.user.username}")
                return Response({
                    'success': True,
                    'data': serializer.data,
        ...
        logger.error(f"Error processing competency: {str(e)}")
        return Response({
            'success': False,
            'error': 'Operation failed'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_weight_config(request):
    ...
    try:
        ...
        if not config:
            return Response({
                'success': True,
                'data': {'sba_weight': 40, 'exam_weight': 60}
            }, status=status.HTTP_200_OK)

        return Response({
            'success': True,
            'data': {'sba_weight': config.sba_weight, 'exam_weight': config.exam_weight}
        }, status=status.HTTP_200_OK)
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    learning_area, _ = LearningArea.objects.get_or_create(
                        area_code=area_code,
                        defaults={'area_name': area_name, 'area_type': 'Core', 'is_active': True}
                    )

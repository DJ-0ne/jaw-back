...
def get_grade_level_from_class(class_obj):
    """Convert numeric_level to actual grade (1-9)"""
...
def get_grading_scale(grade_level):
    """Return appropriate grading scale based on grade level"""
    if grade_level >= 7:  # JSS uses 8-point scale
        return {
    ...
    else:  # Primary uses 4-point scale
        return {
...
def calculate_level_from_score(score, grade_level):
    """Calculate competency level from score based on grade level"""
    if score is None:
...
class CompetencyClassesView(APIView):
    """Get classes for competency matrix"""
...
class CompetencySubjectsView(APIView):
    """Get subjects for competency matrix"""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        try:
            ...
            # Get subjects teacher is allocated to
    ...
    def get(self, request):
        try:
            ...
            return Response({
                'success': True,
                'data': serializer.data,
                'message': f'Found {len(serializer.data)} core competencies'
            })
...
class CompetencyMatrixRetrieveView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Initialize competency_data for all students
            ...
            # If there's a current term, fetch existing ratings
            if current_term:
                academic_year = current_term.academic_year
                
                # Get all portfolio entries for core competencies
                ...
                # Populate competency_data from portfolios
                for portfolio in portfolios:
                    ...
                    if student_key in competency_data and comp_key in competency_data[student_key]:
                        ...
                        if portfolio.evidence_url:
                            if student_key not in evidence_data:
                                evidence_data[student_key] = {}
                            if comp_key not in evidence_data[student_key]:
                                evidence_data[student_key][comp_key] = []
                            evidence_data[student_key][comp_key].append({
                                'id': str(portfolio.id),
            ...
            for comp in competencies:
                ...
                for student in students:
                    student_key = str(student.id)
                    score = competency_data.get(student_key, {}).get(comp_key, {}).get('score')
...
class CompetencyEvidenceView(APIView):
    """Link evidence to student competency"""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        try:
            ...
            # Create or update portfolio with evidence
            portfolio, created = StudentPortfolio.objects.update_or_create(
                student=student,

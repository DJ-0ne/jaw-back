...
def get_teachers(request):
    """
    Get all teachers for class teacher assignment - FIXED to return Staff
    """
    try:
        # Get Staff records with linked User accounts (role=teacher)
...
def update_class(request, class_id):
    try:
        ...
        # FIXED: Handle class_teacher_id as Staff ID
        if 'class_teacher_id' in request.data:
            ...
            if teacher_id:
                try:
                    ...
                    request.data['class_teacher'] = staff
                except Staff.DoesNotExist:
                    return Response({
                        'success': False,
                        'error': 'Selected teacher not found or is not active'
